...
func Run() {
	...
	cfgFile, err := appconfig.Load(appconfig.DefaultPath)
	if err != nil {
		log.Printf("load config: %v\n", err)
	}

	// 既定値
	defAddr := strings.TrimSpace(cfgFile.Addr)
	if defAddr == "" {
		defAddr = a.Preferences().StringWithFallback("addr", ":8080")
	} else {
		a.Preferences().SetString("addr", defAddr)
	}
	...
	if strings.TrimSpace(paths.Run) == "" {
		paths.Run = a.Preferences().StringWithFallback("path.run", "/run")
	}
	if strings.TrimSpace(paths.Tools) == "" {
		paths.Tools = a.Preferences().StringWithFallback("path.tools", "/tools")
	}
	if strings.TrimSpace(paths.Reload) == "" {
		paths.Reload = a.Preferences().StringWithFallback("path.reload", "/reload")
	}
	if strings.TrimSpace(paths.Health) == "" {
		paths.Health = a.Preferences().StringWithFallback("path.health", "/healthz")
	}
	cfgFile.Paths = paths
	defRun := paths.Run
	defTools := paths.Tools
	defReload := paths.Reload
	defHealth := paths.Health
	defKey := cfgFile.APIKey
	if strings.TrimSpace(defKey) == "" {
		defKey = a.Preferences().StringWithFallback("api_key", "devkey")
	} else {
		a.Preferences().SetString("api_key", defKey)
	}
	defCORS := cfgFile.CORSOrDefault()
	a.Preferences().SetBool("cors", defCORS)
	defCORSOrigin := strings.TrimSpace(cfgFile.CORSOrigin)
	if defCORSOrigin == "" {
		defCORSOrigin = a.Preferences().StringWithFallback("cors_origin", "*")
	} else {
		a.Preferences().SetString("cors_origin", defCORSOrigin)
	}
	defListenAll := cfgFile.ListenAllOrDefault()
	a.Preferences().SetBool("listen_all", defListenAll)
	...
	updateConfigFromUI := func() {
		...
		cfgFile.Paths = model.Paths{
			...
			Tools:      strings.TrimSpace(toolsEntry.Text),
			Reload:     strings.TrimSpace(reloadEntry.Text),
			Health:     strings.TrimSpace(healthEntry.Text),
	...
	saveConfig := func() {
		updateConfigFromUI()
		if err := appconfig.Save(appconfig.DefaultPath, cfgFile); err != nil {
	...
	importYAML := func() {
		fd := dialog.NewFileOpen(func(r fyne.URIReadCloser, err error) {
			...
			defer r.Close()
			cfgTmp := appconfig.Default()
			b, _ := io.ReadAll(r)
			if err := yaml.Unmarshal(b, &cfgTmp); err != nil {
			...
			healthEntry.SetText(strings.TrimSpace(cfgFile.Paths.Health))
			keyEntry.SetText(cfgFile.APIKey)
			corsCheck.SetChecked(cfgFile.CORSOrDefault())
			corsOriginEntry.SetText(strings.TrimSpace(cfgFile.CORSOrigin))
